...
class ResultProcess(object):
    ...
    def startProcess(self):
        ...
        while i < len(self.result):
            if (i+3)<len(self.result):
                if self.result[i].split(' ')[0] == '-' and self.result[i + 1].split(' ')[0] == "?" and \
                        self.result[i + 2].split(' ')[0] == "+" and self.result[i + 3].split(' ')[0] == "?":
